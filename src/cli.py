...
from .display import (
    ...
    set_color_scheme, get_current_scheme, list_color_schemes, show_color_schemes,
    show_examples, show_all_commands, show_theme_demo
)
...
def main():
    ...
    # --- theme command ---
    theme_parser = subparsers.add_parser('theme', help='Manage color themes', add_help=False)
    theme_parser.add_argument('action', nargs='?', 
                             choices=['list', 'demo', 'default', 'dark', 'light', 'neon', 'ocean', 'rainbow'], 
                             help='Theme action: list (show all), demo (show current theme demo), or theme name to switch')

    # --- examples command ---
    examples_parser = subparsers.add_parser('examples', help='Show detailed usage examples', add_help=False)

    # --- commands command ---
    commands_parser = subparsers.add_parser('commands', help='Show all available commands', add_help=False)
    ...
    try:
        if args.command == 'list':
            ...
            if args.action == 'list' or args.action is None:
                ...
                show_theme_demo()
            elif args.action in list_color_schemes():
                if set_color_scheme(args.action):
                    show_success(f"已切换到 '{args.action}' 主题并保存设置！")
                    # 显示简单演示
                    show_theme_demo()
                else:
                    show_error(f"无法切换到主题: '{args.action}'")
            else:
                show_error(f"未知的主题: '{args.action}'。使用 'kvs theme list' 查看可用主题。")
